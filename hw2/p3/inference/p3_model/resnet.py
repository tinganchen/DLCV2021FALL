...
from p3_utils.options import args
import torch.utils.model_zoo as model_zoo
from torch.hub import load_state_dict_from_url
from p3_utils import mmd
...
def resnet18_dann(**kwargs):
  return DANN(resnet18)

def resnet34_dann(**kwargs):
  return DANN(resnet34)

def resnet50_dann(**kwargs):
  return DANN(resnet50)
...
class DSAN(nn.Module):
    ...
    def forward(self, source, target, s_label):
        ...
        self.s_pred = self.cls_fc(source)
        if self.training == True and s_label is not None:
            ...
            loss = 0
        return self.s_pred, loss

def resnet18_dsan(**kwargs):
  return DSAN(resnet18)

def resnet34_dsan(**kwargs):
  return DSAN(resnet34)

def resnet50_dsan(**kwargs):
  return DSAN(resnet50)