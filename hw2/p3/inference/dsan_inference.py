import os
import pandas as pd
import p3_utils.common as utils
...
def main():
    ...
    tgt_loader = import_module('p3_data.data').__dict__['Data'](args,  
    ...
    # load training model
    model_t = import_module('p3_model.resnet').__dict__[args.model]().to(device)
...
def inference(args, loader_test, model_t):
    ...
    with torch.no_grad():
        for i, (inputs, targets, data_file) in enumerate(loader_test, 1):
            ...
            logits, _ = model_t(inputs, inputs, None)
